  ...
  return (
    <>
      ...
      <Section id="highlights" className="bg-surface">
        <h2 className="font-display text-3xl">Why People Love Us</h2>
        <ul className="grid md:grid-cols-2 gap-6 mt-6">
          {TESTIMONIALS.slice(0, 2).map((t, i) => (
            <li key={i} className="p-6 rounded-lg bg-white/70 shadow-subtle">
              <p className="italic">“{t.quote}”</p>
              <p className="mt-2 text-sm text-text/70">— {t.author}</p>
            </li>
          ))}
        </ul>
      </Section>
      ...
      <Section className="text-center">
        <h2 className="font-display text-3xl">Ready to taste happiness?</h2>
        <div className="mt-5">
